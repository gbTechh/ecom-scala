...
object OrderView {
  ...
  private def toFrag(value: Any): Frag = value match {
    case s: String => s
        ...
        div(
          ...
          div(
            ...
            div(
              ...
              p(`class` := "card-text", s"Status: ${order.status}"),
              p(`class` := "card-text", s"Total: ${currencyFormat.format(order.totalAmount)}")
            )
          ),
          h3("Order Items"),
          table(
            ...
            tbody(
              items.map { case (item, product) =>
                tr(
                  ...
                  td(toFrag(currencyFormat.format(item.unitPrice * item.quantity)))
                )
              }
            )
          ),
