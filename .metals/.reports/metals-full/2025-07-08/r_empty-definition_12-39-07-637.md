error id: file://<WORKSPACE>/src/main/scala/http/server/Server.scala:`<error>`#`<error>`.
file://<WORKSPACE>/src/main/scala/http/server/Server.scala
empty definition using pc, found symbol in pc: 
empty definition using semanticdb

found definition using fallback; symbol EmberServerBuilder
offset: 2561
  ...
  def stream: IO[Unit] = {
    for {
      ...
      _ <- DatabaseConfig.createTransactor(config).use { xa =>
        ...
        val orderCtrl =
          new OrderController(orderService, cartService, productService)
        ...
        EmberSer@@verBuilder
          ...
          .withPort(port"8080")
          .withHttpApp(routesWithSession.orNotFound)
          .build
          .use(_ => IO.never)
      }
    } yield ()
