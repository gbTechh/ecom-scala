...
### scala.reflect.internal.FatalError: 
  ...
  last tree to typer: Ident(xa)
       tree position: line 34 of file://<WORKSPACE>/src/main/scala/repository/ProductRepository.scala
            tree tpe: doobie.util.transactor.Transactor[cats.effect.IO]
              symbol: value xa
   symbol definition: xa: doobie.util.transactor.Transactor[cats.effect.IO] (a TermSymbol)
      symbol package: repository
       symbol owners: value xa -> method apply -> object ProductRepository
...
== Source file context for tree position ==
    ...
    32         FROM products 
    33         WHERE id = $slu_CURSOR_g AND deleted_at IS NULL
    34       """.query[Product].option.transact(xa)
    35     }
    36 
    37     def create(product: Product): IO[Unit] = {
  ...
  def apply(xa: Transactor[IO]): ProductRepository = new ProductRepository {
    ...
    def findBySlug(slug: String): IO[Option[Product]] = {
      sql"""
        SELECT id, name, slug, description, status, created_at, updated_at, deleted_at 
        FROM products 
        WHERE id = $slu@@g AND deleted_at IS NULL
      """.query[Product].option.transact(xa)
    }
